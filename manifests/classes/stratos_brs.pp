...
class stratos_brs ( $offset=0, $tribes_port=4000, $config_db=governance, $maintenance_mode=true ) inherits system_config {
	...
	file { "/mnt/$server_ip/$service_dir/":
			    ...
			    "puppet:///stratos_brs/configs/",
			    "puppet:///stratos_brs/patches/"],
		sourceselect => all,
                ensure  => present,
		recurse	=> true,
                require => Exec["init_brs"],
        }		
