...
class stratos_manager ( $offset=0, $tribes_port=4000, $config_db=governance, $maintenance_mode=true ) inherits system_config {
	
	$service_code = "manager"
	$service_dir = "wso2stratos-$service_code-$stratos_version"	
	$service_templates = ["registry.xml","axis2.xml","carbon.xml","datasources.properties","advanced/authenticators.xml",
			      "billing-config.xml","email-admin-config.xml","email-billing-notifications.xml","email-new-tenant-activation.xml",
			      "email-new-tenant-registration.xml","email-password-reset.xml","email-payment-received-customer.xml","email-payment-received-wso2.xml",
			      "email-registration-complete.xml","email-registration.xml","email-update.xml","tenant-reg-agent.xml"]
	...
	exec { "init_manager":
		...
		timeout	=> 0,
                require => Exec["remove_manager_poop"],
        }
